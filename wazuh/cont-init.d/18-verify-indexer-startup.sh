...
if [ ! -f "/etc/wazuh-indexer/certs/wazuh-indexer.pem" ]; then
    echo "  ⚠️ Indexer certificates not found - will be generated by init script"
fi

# 4. Verify config file exists
if [ ! -f "/etc/wazuh-indexer/opensearch.yml" ]; then
    echo "  ❌ ERROR: Indexer config file not found!"
    exit 1
fi

# 5. Verify binary exists
if [ ! -f "/usr/share/wazuh-indexer/bin/opensearch" ]; then
    echo "  ❌ ERROR: Indexer binary not found!"
    exit 1
fi
...
echo "✅ Indexer startup prerequisites verified"
