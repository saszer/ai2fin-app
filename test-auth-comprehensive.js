...
// embracingearth.space - enterprise auth testing
...
async function testBearerTokenAuth() {
  ...
  try {
    ...
    if (login.status === 200 && login.data.token) {
      ...
      logTest('OIDC Login & Token', false, `Status: ${login.status}`);
      return null;
    }
  } catch (e) {
...
async function testRefreshScenario(token) {
  ...
  // 1. Frontend loads, checks localStorage for token
  // 2. Makes request to /api/auth/me with Bearer token
  // 3. Should get user data back
